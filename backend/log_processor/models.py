...
#    log_type = models.CharField(max_length=50)
#    timestamp = models.DateTimeField()
#    userAuditID = models.IntegerField() # what is and what for auid? -> refers to audit user ID; it's assigned to user upon login and is inherited by every process even when user's identity changes; useful to trace events when user switches user contexts but you still want to know what they did (https://izyknows.medium.com/linux-auditd-for-threat-hunting-part-2-c75500f591e8)
#    result = models.CharField(max_length=20)
    
#    def __str__(self):
#        return f"{("sucsessful", "failed")[self.result]} change performed by user {self.userAuditID} at {self.timestamp}"
#        # check if statement in formated string works

#class Daemon_Start(models.Model):
#    log_type = models.CharField(max_length=50)
#    timestamp = models.DateTimeField()
...
#    def __str__(self):
#        return f"{("sucsessful", "failed")[self.result]} starting daemon by user {self.userAuditID} at {self.timestamp}"

#class Sys_Config_Change(models.Model):
#    log_type = models.CharField(max_length=50)
#    timestamp = models.DateTimeField()
#    userAuditID = models.IntegerField()
#    result = models.CharField(max_length=20)
...
#    def __str__(self):
#        return f"{systemCall.get(self.syscallValue)} by user {self.userAuditID} at {self.timestamp}"
## comment whether to have the dictionary or not and if yes ask, whether here or somewhere else
